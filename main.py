...
# todo
#  0. Замечания
#  1. Добавить фрукт, который спавнит фигуру (на твой выбор, крест например) из стен на несколько
#  секунд в случайной точке. Как обработать взаимодействие с фруктом и змейкой - на твоё усмотрение (можно респавнить
#  фрукт, можно просто новую точку для стен срандомить)
#  2. Добавить змейку-соперника (стремится к твоим фруктам, тебя избегает, остальные правила идентичны,
#  пересечения разных змеек - тоже смерть).
#   2.1 Добавь просто тело змейки на поле и настрой всё (корректные пересечения меж змейками, не спавн фруктов внутри).
#   Можешь на своё усмотрение, можешь попробовать юзать классы,
#   но придётся много переписать. Если захочешь, то по сути у тебя будет один и тот же класс для змеек,
#   но с разным управлением.
#   2.2 Добавь ей движения, тут два пути: тупой и крутой, можно начать с одного и потом сделать второй.
#   Тупой - просто движется на фрукт (до нужной x коорд., потом до y), ну или что-нибудь похожее. Крутой - ищешь
#   какой-нибудь алгоритм поиска пути и пробуешь его реализовать, только в отдельный файл вынеси,
#   чтоб в мейне не валялось
...
while True:
    ...
    while True:
        if msvcrt.kbhit():
            key = msvcrt.getch()
            # todo странный прекол, использовать для всех биндингов словарь, а для паузы руками проверять, крч плохая
            #  практика
            if key == b'p':
                print('pause')
